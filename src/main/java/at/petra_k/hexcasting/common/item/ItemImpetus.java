...
/** Item form of an impetus, preserving its bound program through placement. */
public final class ItemImpetus extends ItemBlock {
    private static final String TAG_BLOCK_ENTITY = "BlockEntityTag";

    public ItemImpetus(Block block) {
        super(block);
        setMaxStackSize(64);
    }
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world,
        ...
        if (world.getTileEntity(pos) instanceof TileEntityImpetus) {
            ((TileEntityImpetus) world.getTileEntity(pos)).restoreProgram(
                blockEntityTag.getTagList("patterns", 10));
        }
        return true;
    }
}
